...
class InMemoryRetriever(Retriever):
    ...
    def retrieve(self, query_embedding: list[float], top_k: int = 5) -> list[Chunk]:
        ...
        Non-positive ``top_k`` values return an empty list (same contract as
        ``FAISSRetriever`` / ``ChromaRetriever``). An empty index also returns
        an empty list. Invalid query vectors or dimensions raise
        ``RetrieverError``. Non-integer / bool ``top_k`` raises
        ``RetrieverError``.
        """
        if not isinstance(top_k, int) or isinstance(top_k, bool):
            raise RetrieverError("top_k must be an integer.")
        if not (top_k > 0) or not self._chunks or self._matrix is None:
            return []
